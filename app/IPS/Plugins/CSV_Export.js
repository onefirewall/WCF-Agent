...
var CSV_Export = function() {
    ...
    this.init = function(input_obj, callback_array) {
        ...
        console.log('Start csv_export...');
        now = Math.round((new Date()).getTime() / 1000);

        evalScore = new Function('scoreTimeZero', 'current_time', input_obj.eval)

        var wstream = fs.createWriteStream(input_obj.config.ips.csv.csv_logs);
        ...
        input_obj.db.createReadStream().on('data', function(data) {
            ...
            if (data.value[plugin] && data.value[plugin].blocked === true) {
                wstream.write(data.value.ip + "," + score.toString() + '\n');
            }
            ...
            if (batchOps.length == 1000) {
                input_obj.db.batch(batchOps, function(err) {
                    if (err){
                        new ELK_Logs().send_logs(input_obj,0,0,{"m": `ERROR during csv export batch operation ${err}`}, "ERROR")
                        return console.log("ERROR during CSV batch operation " + err);
                    }
                });
                batchOps = []
            }
            ...
            input_obj.db.batch(batchOps, function(err) {
                if (err){
                    new ELK_Logs().send_logs(input_obj,0,0,{"m": `ERROR during CSV batch operation ${err}`}, "ERROR")
            ...
            console.log('Stop CSV - >' + (ts_now4 - ts_now3) + ' ms');
            wstream.end();
            if (something_changed) {
                reload_rules(input_obj, input_obj.config.ips.csv.csv_reload_command)
            }
            console.log('Reloaded CSV - >' + (new Date().getTime() - ts_now4) + ' ms');
        })

    }

    function reload_rules(input_obj, csv_reload_command) {
        try {
            new ELK_Logs().send_logs(input_obj,0,0,{"m": `INFO: Execute Command: ${csv_reload_command}`}, "CSV EXEC COMMAND")
            exec(csv_reload_command, (error, stdout, stderr) => {
                if (error !== null) {   
                    new ELK_Logs().send_logs(input_obj,0,0,{"m": `ERROR: exec CSV error: ${error}`}, "ERROR")
                }
            });
        } catch (err) {
            console.error(err)
